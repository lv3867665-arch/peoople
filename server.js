...
// Valida se um valor bruto da célula pode ser um telefone brasileiro
function extractBrazilianPhone(raw) {
    if (raw === null || raw === undefined) return null;
    const digits = String(raw).replace(/\D/g, '');
    // Aceita entre 10 e 13 dígitos (com ou sem código do país, com ou sem o 9 adicional)
    if (digits.length < 10 || digits.length > 13) return null;
    const normalized = normalizePhone(digits);
    // Deve começar com 55 e ter 12 dígitos após normalização
    // (55 + DDD 2 dígitos + 8 dígitos do número)
...
app.post('/api/excel/parse', (req, res) => {
    try {
        ...
        // Percorre todas as abas e todas as células
        for (const sheetName of workbook.SheetNames) {
            const sheet = workbook.Sheets[sheetName];
            const rows = XLSX.utils.sheet_to_json(sheet, { header: 1, raw: true, defval: null });
            for (const row of rows) {
                for (const cell of row) {
                    const phone = extractBrazilianPhone(cell);
...
app.post('/api/leads/batch', async (req, res) => {
    try {
        ...
        const now = new Date();
        const ops = phones.map(p => {
            const phone = normalizePhone(String(p));
            return {
                updateOne: {
                    filter: { phone },
                    update: { $setOnInsert: { phone, name: '', tags: [], updatedAt: now } },
                    upsert: true
                }
            };
        });
        ...
        res.json({ success: true, saved: phones.length });
    } catch (e) {
